...
import requests
# The URL to fetch the SEC filing page
url = 'https://www.sec.gov/edgar/browse/?CIK=1045810&owner=exclude'
# Send a GET request to the URL
response = requests.get(url)
# Check if the request was successful (status code 200)
if response.status_code == 200:
    # Open a file in write mode and save the response text (HTML content)
    with open('0test.html', 'w', encoding='utf-8') as file:
        file.write(response.text)
    print("HTML content saved successfully to 'sec_filing_page.html'")
else:
